...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    <div
      className={`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r boder-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md: absolute left-0 z-1 ${!isMenuOpen && "max-md:translate-x-full"}`}
    >
      {/* LOGO */}
      ...
      <div className="flex-1 overflow-y-scroll mt-3 text-sm space-y-3">
        {chats
          .filter((chat) =>
            chat.messages[0]
              ? chat.messages[0]?.content.toLowerCase().includes(search.toLocaleLowerCase())
      ...
      <div className="flex items-center justify-between gap-2 mt-4 p-2 border border-gray-300 dark:border-white/15 rounded-md ">
      ...
      <div className="flex items-center gap-3 mt-4 p-3 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer  group">
        ...
        {user && (
          <img src={assets.logout_icon} className="h-5 cursor-pointer hidden not-dark:invert group-hover:block" />
        )}
      </div>
      ...
      <img
        ...
        className="absolute top-3 right-3 size-5 cursor-pointer md:hidden not-dark:invert"
        alt=""
      />
    </div>
  );
